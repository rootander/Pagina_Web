...
class Config:
    ...
    CAMERA_WIDTH = 640
    CAMERA_HEIGHT = 480
    # Umbral del porcentaje de frames que deben coincidir (0-1)
    FACE_MATCH_THRESHOLD = 0.6
    # Duración de la ventana de autenticación facial (segundos)
    LOGIN_DURATION = 10
    # Tolerancia para la comparación de encodings (menor = más estricto)
    FACE_TOLERANCE = 0.6
...
class ModernButton(tk.Canvas):
    def __init__(self, parent, text, command, width=200, height=40, bg=Config.PRIMARY_COLOR, fg="white", font=Config.BUTTON_FONT, hover_bg=Config.SECONDARY_COLOR, *args, **kwargs):
        ...
        # Guardar id del rectángulo para modificar solo fondo
        self.rect_id = Utilities.create_rounded_rectangle(
            self, 0, 0, width, height, radius=10, fill=bg, outline=""
        )
        
        # Guardar id del texto para no modificar color en hover
...
class FacialRecognitionSystem:
    ...
    def show_login_screen(self):
        ...
        # Guardar referencia a la ventana de login para cerrarla después
        ...
        def update_frame():
            if self.cap is not None and self.cap.isOpened():
                ...
                if ret:
                    ...
                    # Escalar las ubicaciones de nuevo al tamaño original
                    scale_factor = 2
            ...
            if video_label.winfo_exists():  # Verificar que el widget aún existe
    ...
    def capture_face(self, username, window):
        if self.cap is not None and self.cap.isOpened():
            ...
            if ret:
                ...
                # Tomamos el primer rostro (más prominente)
    ...
    def start_facial_login(self, video_label, timer_label, status_label, percent_label, reference_encoding, username, login_window):
        ...
        def update_frame():
            ...
            # Finalizar cuando pase el tiempo
            if elapsed >= Config.LOGIN_DURATION:
                ...
                if ratio >= Config.FACE_MATCH_THRESHOLD:
                    ...
                    # Cerrar la ventana de login principal si existe
